...
const { buyTollRoadCard, topUpBalance, useTollRoad } = require('./utils');
  ...
  async function getTollAccess() {
    try {
      const beliKartuTol = await buyTollRoadCard(25);
      const isiSaldoKartuTol = await topUpBalance(beliKartuTol, 10);
      const gunakanAksesJalanToll = await useTollRoad(isiSaldoKartuTol);
  }
    catch(error){
